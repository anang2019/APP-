...
;!function(win){
    ...
    var Laykefu = function(){
        ...
        isMobile();
        isVisible();
        socket = new WebSocket('wss://' + config.socket);//创建Socket实例
        ...
        content = (content || '').replace(/&(?!#?[a-zA-Z0-9]+;)/g, '&amp;')
            ...
            .replace(/mp3\[([^\s]+?)\]/g, function (mp3) {  // 转义mp3
                return '<audio controls="controls">\n' +
                    '      <source src="' + mp3.replace(/(^mp3\[)|(\]$)/g, '') + '" type="audio/mpeg">\n' +
                    '  </audio>';
            })
            .replace(/mp4\[([^\s]+?)\]/g, function (mp4) {  // 转义mp3
                return '<video width="320" height="240" controls>\n' +
                    '                <source src="' + mp4.replace(/(^mp4\[)|(\]$)/g, '') + '" type="video/mp4">\n' +
                    ' </video>';
            })
        ...
        var _html = '<img src="" alt="" class="laykefu-bigimg" style="display:none; inline;width: 80% !important;z-index:9999999999"><div class="laykefu-mask" style="display: none;z-index: 9999999;"></div>';
        $('body').append(_html);
        ...
        $('#laykefu-up-image').after('<form enctype="multipart/form-data"><input type="file" id="laykefu-upload" name="file" accept="image/*" style="display:none" ></form>');
        ...
        var imgType = ["image/jpeg","image/png","image/gif","image/jpg","image/heic","image/bmp"];
