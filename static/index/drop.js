...
var upfile=fileList[0];$("#dialog-uploadify").show();
...
if(upfile.size<1048576){
	...
	var _fixed=upfile.size/1048576;var _size=_fixed.toFixed(2)+"MB";
	
}
if(upfile.name.length>10){
	var _name=upfile.name.substr(0,10)+"...";
	
}else{var _name=upfile.name;
...
app_xhr.upload.onloadstart=function(evt){
	app_ot=new Date().getTime();app_oloaded=0;};app_xhr.send(fd);},false);});

 	function  Alert(j){

		$('body').append('<div class="white"></div><div class="alert-news">'+j+'</div>')
