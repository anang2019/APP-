...
function each_confirm() {
    $.layer({
        ...
        area: ["auto", "auto"],
        dialog: {
            msg: "确定要解除合并吗？", btns: 2, type: 4, btn: ["确定", "取消"], yes: function () {
                each_del();
            }, no: function () {
                layer.msg("已取消解除", 1, 0);
            }
...
function del_app(_id, _type) {
    if (_type > 0) {
        $.layer({
            ...
            area: ["auto", "auto"],
            dialog: {
                msg: "删除操作不可逆，确认继续？", btns: 2, type: 4, btn: ["确认", "取消"], yes: function () {
                    del_app(_id, 0);
                }, no: function () {
                    layer.msg("已取消删除", 1, 0);
                }
...
function remove_ad(_id, _type) {
    if (_type > 0) {
        $.layer({
            ...
            area: ["auto", "auto"],
            dialog: {
                msg: "去除需扣除 " + in_adpoints + " 下载点数？", btns: 2, type: 4, btn: ["确认", "取消"], yes: function () {
                    remove_ad(_id, 0);
                }, no: function () {
                    layer.msg("已取消去除", 1, 0);
                }
