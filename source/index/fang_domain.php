<?php if (!defined('IN_ROOT')) {
    exit('Access denied');
} ?>
<?php if (!$GLOBALS['userlogined']) {
    exit(header('location:' . IN_PATH . 'index.php/index'));
} ?>
<?php
// 登录id
...
if ($_POST['addfangfeng']) {
    ...
    if ($title) {
        ...
        if (empty($query22233)) {
            if ($beborn) {
                if ($end_time) {
                    $GLOBALS['db']->query("insert INTO t_jump (entrance,title,status,beborn,end_time,remarks,user_uid,create_time) VALUES ('$entrance','$title','1','$beborn','$end_time','$remarks','$userid','$create_time')");
...
if ($_POST['updfangfeng']) {
    ...
    $GLOBALS['db']->query("update t_jump set entrance='$entrance', title='$title' , beborn='$beborn' , end_time='$end_time' where id=" . $id);
            ...
            <div class="middle-wrapper">
                <sidebar class="avatar-dropdown">
                    ...
                    <div class="name"><span class="ng-binding">
									<?php echo substr($GLOBALS['erduo_in_username'], 0, strpos($GLOBALS['erduo_in_username'], '@')); ?></span>
                    </div>
                    ...
                    <div class="dropdown-menus">
                        <ul>
                            ...
                            <li><a href="<?php echo IN_PATH . 'index.php/profile_info'; ?>" class="ng-binding">个人资料</a>
                            </li>
                ...
                <nav>
                    ...
                    <div class="navbar-title primary-title"><a href="<?php echo IN_PATH . 'index.php/fang_add'; ?>"
                                                               class="ng-binding" style="color: #000;">我的防封</a></div>
                    <div class="navbar-title primary-title"><a href="<?php echo IN_PATH . 'index.php/home'; ?>"
                                                               class="ng-binding">我的分发</a></div>


                    <div class="navbar-title primary-title"><a href="http://app.92ff.cn" target="view_window"
                                                               class="ng-binding" style="color: #F44336 !important;">
                            <img style="    width: 22px;position: absolute;transform: rotate(-42deg);margin-top: -9px;margin-left: -13px;"
                                 src="<?php echo IN_PATH; ?>static/index/huangguan1.png">超级签名</a></div>
                </nav>
            </div>
        </div>
    </div>
        ...
        <div data-v-92459f82="" class="app-container">
            <div class="filter-container">
                <form action="" method="post">
                    ...
                    <a href="/index.php/fang_add">
                        ...
                        <button type="button"
                                class="el-button filter-item el-button--primary el-button--medium display2"
            ...
            <div class="el-table el-table--fit el-table--border el-table--scrollable-x el-table--enable-row-hover el-table--enable-row-transition el-table--medium"
                    ...
                    <table cellspacing="0" cellpadding="0" border="0" class="el-table__header" style="width: 1110px;">
                        ...
                        <tr class="">
                            ...
                            <th colspan="1" rowspan="1"
                                class="el-table_1_column_7  is-center small-padding fixed-width  is-leaf">
                ...
                <div class="el-table__body-wrapper is-scrolling-left" style="overflow:hidden">
                    <table cellspacing="0" cellpadding="0" border="0" class="el-table__body" style="width: 1332px;">
                        ...
                        //------------分页开始-------------------
                        //1.求出总条数
                        $count = $GLOBALS['db']->getone("select count(*) from t_domain where status<2 and pid=$userid ");
                        //2.每页显示数(5)
                        $num = 10;
                        ...
                        $obj =$GLOBALS['db']->query("select * from t_domain  where status<2 and pid=$userid limit " . $offset . ',' . $num);
                        ?>
                        <!---->
                        <tbody>
                        <?php
                        while ($row = $GLOBALS['db']->fetch_array($obj)) {
                            ...
                            <tr class="el-table__row">
                                ...
                                <td rowspan="1" colspan="1" class="el-table_1_column_4 is-center ">
                                    <div class="cell"><span><? echo date('Y-n-j H:i:s', $row['buy_time']) ?></span>
                                    </div>
                                </td>
                                    ...
                                    <div class="cell">
                                        ...
                                        if ($row['status'] == 1) {
                                            ...
                                            <span class="el-tag el-tag--success el-tag--medium"
                                                  style="background-color: #ff49491c; border-color: #ff4949; color: #ff4949;">已屏蔽</span>
                                            <?php
                                        }
                                        ?>
                                ...
                                <td rowspan="1" colspan="1" class="el-table_1_column_5 is-center ">
                                    ...
                                    <select data-fangfengyumingid="<? echo $row['id'] ?>"
                                            class="fangfengyumingduoxuankuang"
                                            style="width:80%;border:none;text-align:center;">
                                        ...
                                        if ($row['remarks']) {
                                            $query5 = $GLOBALS['db']->query("select * from t_jump where user_uid=" . $userid);
                                            while ($row5 = $GLOBALS['db']->fetch_array($query5)) {
                                        ...
                                        $query6 = $GLOBALS['db']->query("select * from t_jump where user_uid=" . $userid);

                                        while ($row6 = $GLOBALS['db']->fetch_array($query6)) {
                                            ?>
                                            <?php
                                ...
                                <td rowspan="1" colspan="1"
                                    class="el-table_1_column_7 is-center small-padding fixed-width">
                                    <!--a2-->
                                    <div class="cell">
                                        ...
                                        if ($row['status'] == 1) {
                                            ...
                                            if ($row['remarks']) {
                                                ...
                                                <button type="button"
                                                        style="background-color: #ebeceecf;border-color: #ebeceecf;"
                                                ...
                                                ?>
                                                <button type="button"
                                                        onclick="fangfengyumingupda(<? echo $row['id'] ?>)"
                                                        class="el-button el-button--primary el-button--mini ">
                                                    <span>保存</span>
                                                </button>
                                                <?php
                                            ...
                                            <button type="button" onclick="shanchuyuming(<? echo $row['id'] ?>)"
                                                    class="el-button el-button--default el-button--mini shanchu">
                                                <span>删除</span>
                                            </button>
                                            <?
                                        }
            ...
            <div data-v-6af373ef="" class="pagination-container">
                ...
                <div data-v-6af373ef="" class="el-pagination is-background">
                    ...
                    <span class="el-pagination__total">共 <?php echo $count ?> 条</span><span
                            class="el-pagination__sizes">
                     <a href="/index.php/fang_domain"><<</a> 
        	<a href="/index.php/fang_domain?page=<?php echo $prev; ?>"><button type="button" class="btn-prev"><i
                            class="el-icon el-icon-arrow-left"></i></button></a>
                    <ul class="el-pager">
                        <li class="number active"><? echo $page ?></li>
                    </ul>
                    
            <a href="/index.php/fang_domain?page=<?= $next; ?>"><button type="button" class="btn-next"><i
                            class="el-icon el-icon-arrow-right"></i></button></a>
            ...
            if ($(".fangfengyumingduoxuankuang").eq(i).data("fangfengyumingid") == val) {
                ...
                if (index != "请选择") {
                    layer.confirm('提交后不能更改,您确定提交吗？', function () {
                        $.post("/index.php/phpapi", {
                            xuanzeid: index,
                            yumingid: val,
                            act: 'upd_fangfengyumingupdate'
                        }, function (data1) {
